...
export class PdfExportService {
  ...
  async exportOrderInvoice(order: Order): Promise<void> {
    ...
    doc.text(`Fecha: ${formatDate(new Date(), 'dd/MM/yyyy HH:mm', 'en-US')}`, pageWidth - 14, 18, {
      align: 'right',
    });
    ...
    const tableData = (order.items || []).map((item) => [
      ...
      this.currencyPipe.transform(item.cantidad * item.precio_unitario) ||
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(
      `TOTAL: ${this.currencyPipe.transform(order.total) || `$${order.total}`}`,
      pageWidth - 14,
      finalY,
      { align: 'right' },
    );
